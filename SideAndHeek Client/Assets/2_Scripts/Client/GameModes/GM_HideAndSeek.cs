...
public class GM_HideAndSeek : GameMode
{
    ...
    public override void GameStart()
    {
        foreach (Player player in Player.list.Values)
        {
            ...
            {
                GameManager.instance.StartCoroutine(SpawnSpecial(player, (int)CustomGameRules.hidingTime));
            }
        }
    }
    ...
    public override void OnPlayerCollision(Player player, Player other)
    {
        if (other.playerType == player.playerType)
        {
            if (player.playerMotor.GetCanKnockOutOthers())
            {
                ...
                {
                    other.playerMotor.OnCollisionWithOther(3f);
                }
            }
        }
        else if (player.playerType == PlayerType.Hunter)
        {
            if (!player.activePlayerCollisionIds.Contains(other.Id))
            {
                ...
                if (CustomGameRules.catchType == CatchType.OnTouch || (CustomGameRules.catchType == CatchType.OnFlop && player.playerMotor.GetCanKnockOutOthers()))
                {
                    if (other.isBodyActive)
                    {
                        other.playerMotor.OnCollisionWithOther(3f);
    ...
    public override void OnPlayerTypeSet(Player player, PlayerType playerType, bool isFirstHunter)
    {
        ...
        player.playerMotor.SetForwardForceMultiplier(speedMultiplier);
    }
