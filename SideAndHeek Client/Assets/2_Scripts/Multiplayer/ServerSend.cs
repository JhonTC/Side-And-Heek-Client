...
public class ServerSend
{
    ...
    public static void PlayerPositions(Player _player, SimplePlayerController _movementController)
    {
    ...
    public static void PlayerRotations(Player _player, SimplePlayerController _movementController)
    {
        if (_player.isBodyActive)
        {
            Message message = Message.Create(MessageSendMode.Unreliable, ServerToClientId.playerRotation);
            message.AddUShort(_player.Id);
            message.AddQuaternion(_movementController.root.rotation);

            NetworkManager.Instance.Server.SendToAll(message);
        }
    }

    public static void PlayerState(Player _player, PlayerMotor _motor, SimplePlayerController _movementController)
    {
        if (_player.isBodyActive)
        {
            Message message = Message.Create(MessageSendMode.Unreliable, ServerToClientId.playerState);
            message.AddUShort(_player.Id);
            message.AddBool(_movementController.largeGroundCollider.isGrounded);
            message.AddBool(_player.headCollided);
            message.AddFloat(_movementController.root.velocity.magnitude);
            message.AddFloat(_motor.inputSpeed);
            message.AddBool(_player.footCollided);

            message.AddBool(_movementController.isJumping);
            message.AddBool(_movementController.isFlopping);
            message.AddBool(_movementController.isSneaking);
    ...
    public static void CreatePickupSpawner(ushort _spawnerId, Vector3 _position, ushort _playerId, Pickup _pickup = null)
    {
        ...
        if (_pickup != null)
        {
            ...
            message.AddInt((int)_pickup.activeObjectDetails.pickupSO.pickupCode);
        }
    ...
    public static void SetVisualEffect(ushort id, VisualEffect effect)
    {
        Message message = Message.Create(MessageSendMode.Reliable, ServerToClientId.setVisualEffect);

        message.AddUShort(id);
        message.AddBool(effect != null);
        if (effect != null)
        {
            //effect.AddMessageValues(ref message);
        }
